...
public class SubTaskUpdatedNotificationHandler : INotificationHandler<SubTaskUpdatedEvent>
{
    private readonly IPMDbContext _dbContext;
    private readonly INotificationService _notificationService;

    public SubTaskUpdatedNotificationHandler(IPMDbContext dbContext, INotificationService notificationService)
    {
    ...
    {
        var assigneeChange = notification.ChangedFields.FirstOrDefault(c => c.FieldName == "AssigneeUserId");
        if (assigneeChange == null || string.IsNullOrEmpty(assigneeChange.NewValue))
            return;

        if (Guid.TryParse(assigneeChange.NewValue, out var newAssigneeId) && newAssigneeId != notification.UserId)
        {
